...
class GPT2Model(GPTPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    # each token id gets mapped to a learned 768-dim vector
    # e.g. the token for "hello" gets its own unique embedding
    token_embeds = self.word_embedding(input_ids)

    # we also need to encode position info (where each token is in the sentence)
    # without this, "the cat sat" and "sat cat the" would look the same
    pos_ids = self.position_ids[:, :seq_length]
    pos_embeds = self.pos_embedding(pos_ids)

    # add token and position embeddings together, then apply dropout
  ...
  def hidden_state_to_token(self, hidden_state):
    ...
    """
    # weight tying: reuse the same embedding matrix from the input side
    # multiply hidden state by E^T to get a score for every word in the vocab
    # whichever word has the highest score is the model's prediction
