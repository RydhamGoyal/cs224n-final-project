...
class ParaphraseGPT(nn.Module):
  ...
  def __init__(self, args):
    ...
    if self.use_lora:
      # freeze pretrained weights and inject LoRA into Q and V projections
  ...
  def forward(self, input_ids, attention_mask):
    """cloze-style paraphrase classification using yes/no token embeddings."""
    output = self.gpt(input_ids, attention_mask)
    last_token_hidden = output['last_token']
    # get embedding vectors for "no" (3919) and "yes" (8505)
    # detach so we don't backprop through the embedding matrix
    yes_no_weights = self.gpt.word_embedding.weight[[3919, 8505]].detach()  # [2, 768]
    yes_no_logits = last_token_hidden @ yes_no_weights.T  # [batch_size, 2]
...
def train(args):
  ...
  # print parameter counts so we can compare full vs LoRA
  total, trainable = count_parameters(model)
  print(f"Total params: {total:,} | Trainable params: {trainable:,} ({100*trainable/total:.2f}%)")

  lr = args.lr
  # only optimize parameters that require gradients (matters for LoRA)
...
def test(args):
  ...
  # map 0->3919 (no) and 1->8505 (yes) for autograder
  label_map = {0: 3919, 1: 8505}

  with open(args.para_dev_out, "w+") as f:
    f.write(f"id \t Predicted_Is_Paraphrase \n")
    for p, s in zip(dev_para_sent_ids, dev_para_y_pred):
      f.write(f"{p}, {label_map[s]} \n")
...
if __name__ == "__main__":
  args = get_args()
  if args.use_lora:
    args.filepath = f'{args.epochs}-{args.lr}-lora-r{args.lora_r}-paraphrase.pt'
  else:
